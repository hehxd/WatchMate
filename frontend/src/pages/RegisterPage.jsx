...
export default function RegisterPage({setView}) {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setError('Registration failed. That email or username might already be in use.');
            }
        } catch (err) {
    ...
    return (
        <div className="w-full max-w-md mx-auto text-center animate-fade-in">
            ...
            {error &&
                <p className="text-red-500 mb-4 bg-red-500/10 p-3 rounded-xl border border-red-500/20 text-sm font-bold">{error}</p>}
            {success &&
                <p className="text-green-500 mb-4 bg-green-500/10 p-3 rounded-xl border border-green-500/20 text-sm font-bold">Account
                    created successfully! Redirecting to login...</p>}
